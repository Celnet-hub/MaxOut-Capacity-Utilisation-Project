...
from middleware.models.database import (
    Employee,
    CustomerAccount,
    Contact,
    CustomerProduct,
...
def seed_database():
    ...
    try:
        ...
        if db.query(Employee).first():
            logger.info(
                "Database is already seeded. Drop tables if you want a fresh start."
            )
            return
        ...
        acme_corp = CustomerAccount(
            account_name="Acme Corp UK",
            account_status="Active",
            account_manager_id=am_1.id,
            cx_manager_id=cx_1.id,
        )
        globex = CustomerAccount(
            account_name="Globex Telecommunications",
            account_status="Active",
            account_manager_id=am_1.id,
            cx_manager_id=cx_1.id,
        )
        ...
        contact_1 = Contact(
            ...
            first_name="Charlie",
            last_name="Brown",
            email_address=random.choice(ALLOWED_EMAILS),
        ...
        logger.info(
            f"Use this CID for your API testing: {prod_1.cid} (Provisioned: {prod_1.provisioned_bandwidth_mbps} Mbps)"
        )
