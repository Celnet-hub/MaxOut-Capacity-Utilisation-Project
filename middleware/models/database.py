...
class Employee(Base):
    ...
    )  # in production email should be unique
    department = Column(String, nullable=False)
    status = Column(String, nullable=False)  # e.g., 'Active', 'Terminated'
    employment_date = Column(Date, nullable=False)

    # Relationships
    accounts_managed = relationship(
        "CustomerAccount",
        foreign_keys="CustomerAccount.account_manager_id",
        back_populates="account_manager",
    )
    cx_accounts = relationship(
        "CustomerAccount",
        foreign_keys="CustomerAccount.cx_manager_id",
        back_populates="cx_manager",
    )
...
class CustomerAccount(Base):
    ...
    account_manager = relationship(
        "Employee", foreign_keys=[account_manager_id], back_populates="accounts_managed"
    )
    cx_manager = relationship(
        "Employee", foreign_keys=[cx_manager_id], back_populates="cx_accounts"
    )
...
class CustomerProduct(Base):
    ...
    burst_capacity_mbps = Column(
        Float, nullable=False
    )  # Default to 0 if no burst allowed
