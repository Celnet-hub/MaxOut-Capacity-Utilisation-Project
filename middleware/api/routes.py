...
from schemas.netboss_network import TelemetryCreate, TelemetryResponse
from models.database import CustomerProduct
...
def ingest_telemetry(payload: TelemetryCreate, db: Session = Depends(get_db)):
    ...
    product = db.query(CustomerProduct).filter(CustomerProduct.cid == payload.circuit_id).first()
    ...
    if payload.utilization_mbps >= threshold:
        print(f"⚠️ WARNING: {payload.circuit_id} is maxing out! ({payload.utilization_mbps} Mbps / {product.provisioned_bandwidth_mbps} Mbps)")
